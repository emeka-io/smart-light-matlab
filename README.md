...
This ia an energy-efficient smart lighting project that uses a photoresistor (LDR) to automate LED states. This project bridges hardware and software by using **MATLAB** to visualize millisecond light fluctuations and **Arduino** for embedded control.
...
*This graph shows the LDR value against the my run time. Using my threshold range as a reference. The **Red Dashed Line** represents my 'ON Threshold' while **Green Dashed Line** represents my 'OFF Threshold' (it saves energy when it is off). When the white line (current light level) crosses these thresholds, the system triggers the LED state change.*
